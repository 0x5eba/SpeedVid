document.addEventListener('DOMContentLoaded', function () {
    ...
    function changeHandlerSpeed(event) {
        ...
        chrome.tabs.query({currentWindow: true, active: true}, function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, {type: 'changeSpeed', "speed": curr_speed}, function(resp){})
        })
    }
    function changeHandlerVideo(event) {
        curr_video = this.value
        chrome.tabs.query({currentWindow: true, active: true}, function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, {type: 'changeVideo', "video": curr_video}, function(resp){})
        })
    }
